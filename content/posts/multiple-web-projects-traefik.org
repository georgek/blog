...
I work on multiple web projects and run local development servers for each of them.  I
often have to switch between them and sometimes it's useful to have multiple projects up
at the same time.  Recently I've found a way that makes managing these more enjoyable.
...
Fortunately Docker Compose does have a solution for (2) in the form of the (not very
well documented) ~compose.override.yaml~.  This file will automatically will
automatically be [[https://docs.docker.com/compose/multiple-compose-files/merge/][merged]] into the ~compose.yaml~.

Unlike some other guides concerning this file, I prefer to *not* check
~compose.override.yaml~ into version control and instead add it to the ~.gitignore~
file. Adding it to version control completely defeats the purpose of it: to allow
individual developers to override the standard compose file.
...
This doesn't quite solve our problem yet.  We don't want all of our project inside one
...
Now, you should be able to browse to [[http://proj.traefik.me/]] in your web browser.
...
must start your traefik composition already).  Next we override the ~networks~ setting
of ~web~ to make it part of the ~traefik~ network too.  Note we also have to add the
...
and create their own easily-manageable configurations.
