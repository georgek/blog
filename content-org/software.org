...
developers have never programmed without git and they see git simply as the way to get
new code into a repository. A glorified copy, essentially---but an annoying one that is
...
~important-document-v6-2024-02-16-(gpk).docx~
...
1. Type up the first version of a document, say ~important-document.docx~,
2. Make a copy of that, called ~important-document-v1.docx~,
3. Continue making further additions/edits to ~important-document.docx~,
4. Make another copy of that, called ~important-document-v2.docx~.

The important thing here is discipline. For this to go well, the ~v1~, ~v2~ documents
must never be edited again, lest you undermine the whole system. To make it easier to do
...
└── .vcs
    ├── important-document-v1.docx
    └── important-document-v2.docx
#+end_src
...
go through the corrected ~v6~ and apply all the changes to ~v7~. People just kept this
stuff in their head and, for the most part, it kind of worked.
...
of the entire commit as the version number. But since hashes are not sequential, it also
stores a link to the previous version with every version.
...
In the dumb system we copy the working copy into the ~.vcs~ directory:
...
~HEAD~. You can check this any git repository by running ~cat .git/HEAD~. You would
...
So why use git at all then? Let's look at what git can do beyond the dumb system.
...
like "Your branch is ahead of 'my-remote/my-branch' by 1 commit." You periodically sync
with the remote using ~git fetch~ to see how far behind the upstream branch you are
getting.
...
commit that you haven't yet seen. Over time the upstream will get more commits. The
...
and end up with a difficult merge before you can finish your work.
...
reasons why it's useful to point a branch at any other commit.
...
again. This achieves the same thing as a rebase and the choice is a personal preference.
...
at a lower level with no preconceptions of what version control is which, I think, is
