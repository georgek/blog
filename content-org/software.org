...
#+INCLUDE: ../assets/js/map.js src js

This file won't work on its own; it is supposed to be run through Hugo's asset pipeline,
which we'll set up next.

** Building with Hugo

We are going to use several features of Hugo to make it easy to include a map in any
page on your site.

If you are unfamiliar with Hugo Pipes you can read all about it [[https://www.regisphilibert.com/blog/2018/07/hugo-pipes-and-asset-processing-pipeline/][here]].

FIXME: What happens if more than one page has a map?  The hugo params need to be in a
small per-page script that calls the static script to create a map.
