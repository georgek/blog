...
* TODO Custom Static Vector Maps on your Hugo Static Site    :hugo:blog:maps:
:PROPERTIES:
...
#+hugo: {{<map tiles-url="/blog/gran-canaria2.pmtiles" bounds="-15.923996,27.713926,-15.308075,28.205793" max-bounds="-16.273499,27.508271,-14.889221,28.386568">}}
...
Although I'm using Hugo as a concrete example, all of this should be easily translatable
to any static site.
...
Now you can simply use the shortcode like so:
...
I can't believe how easy this has been for me to set up.  I have previously tried
setting up my own custom maps and found it quite difficult to get started, not to
mention requiring me to run a special tileserver somewhere.  I'm by no means a map
expert (although I am an OpenStreetMap contributor of many years).

I really like that I can now have maps on a truly static site.  The map being static
does have drawbacks, though, and isn't suitable for everything.  In particular, it will
never receive updates unless you update the pmtiles file.  But, for me, this is by
design.  I /want/ these pages to be static, including the map.  If I include a route
showing where I walked, it doesn't make sense for it to appear on some map of the
future.  It /should/ be a map of the past.

Also, let's not forget that maps don't have to contain "real" data.  It could contain a
planned development or even just a fantasy world.  There are many possibilities.  Next I
want to try to get hillshading/relief into my maps.

Just for fun, here's another map:
...
** Notes about org-mode and ox-hugo
